...
async function loadDynamicData() {
    ...
    // === STEP 2: Parallel queries (much faster) ===
    try {
        ...
        // Fallback to individual queries if Promise.all fails
        try {
...
function productCardHTML(p) {
    ...
    // Add cache-control parameter to image URLs for better browser caching
...
function updateFeaturedProducts() {
    ...
    // === CRITICAL FIX: Clear timer and stale data ===
    ...
    var visibleSlides = Math.min(CONFIG.FEATURED_CAROUSEL_VISIBLE || 3, feat.length);
...
function moveFeaturedCarousel(direction) {
    ...
    // Safely wrap within valid range
